...
from komi.utilities import (
    LowerTriangularParam,
    PositiveDiagonalParam,
    UpperTriangularParam,
...
class NonfactoredMultitaskGaussianLikelihood(_MultitaskGaussianLikelihoodBase):
    ...
    def _shaped_noise_covar(
        ...
        task_var_lt = task_var_lt.expand(
            *shape[:-2], *task_var_lt.matrix_shape
        )  # pyre-ignore[6]
...
class LMCMixingMatrix(torch.nn.Module):
    ...
    def __init__(
        self, Q_plus: Tensor, R: Tensor, bulk: bool = True, diagonal_R: bool = False
    ):
        """
        Args:
        ...
        if len(Q_plus.shape) != len(R.shape):
            raise ValueError(
                "Q_plus and R have different number of axes: {0} and {1}".format(
                    Q_plus.shape, R.shape
                )
            )
        else:
        ...
        if Q_plus.shape[self.n_batch_dims + 1] == Q_plus.shape[self.n_batch_dims + 0]:
            ...
            raise ValueError(
                "Wrong dimensions for Q_plus : should be (n_batch x) n_tasks x n_tasks or (n_batch x) n_tasks x n_latents,"
                "got {0}. n_latents has been infered from R to be {1}".format(
                    Q_plus.shape, R.shape[self.n_batch_dims + 0]
                )
            )
        ...
        if bulk:
            if self.mode == "Q_plus":
                R_padded = torch.broadcast_to(
                    torch.eye(self.n_tasks),
            ...
            self.register_parameter(
                "Q_plus", torch.nn.Parameter(Q_plus, requires_grad=True)
            )
...
class ProjectedGPModel(ExactGPModel):
        ...
        **kwargs,
        ...
        if mean_type is not gp.means.ZeroMean:
            raise NotImplementedError(
                "Projected GP model does not support non-zero output-wise means for now !"
            )
        ...
        if proj_likelihood is not None and proj_likelihood.noise.shape[-1] != n_latents:
            raise ValueError(
                "In projected GP model the dimension of the likelihood is the number of latent processes. "
                "Provided likelihood has length {0} while n_latents is {1}".format(
                    proj_likelihood.noise.shape[-1], n_latents
                )
            )
        ...
        if (
            scalar_B and BDN
        ):  # !!! Very structuring choice, corresponding to PLMC-fast ; see PLMC article
            U, S, V = compute_truncated_svd(
                Y=train_y,
                n_latents=n_latents,
                last_target_dim_is_datapoint=last_target_dim_is_datapoint,
            )
        ...
        if not bulk:
            lmc_coefficients = torch.nn.utils.parametrizations.orthogonal(
                lmc_coefficients,
                name="Q_plus",
                orthogonal_map=ortho_param,
                use_trivialization=(ortho_param != "householder"),
            )  # parametrizes Q_plus as orthogonal
            if diagonal_R:
                torch.nn.utils.parametrize.register_parametrization(
                    lmc_coefficients, "R", PositiveDiagonalParam()
                )
            else:
                torch.nn.utils.parametrize.register_parametrization(
                    lmc_coefficients, "R", UpperTriangularParam()
                )
        ...
        if scalar_B:
            ...
            self.register_parameter(
                "log_Sigma_orth",
                torch.nn.Parameter(log_init_noise * discarded_noise_tens[..., :1]),
            )
            self.register_constraint(
                "log_Sigma_orth", gp.constraints.GreaterThan(log_noise_thresh)
            )
            if BDN:
                self.register_buffer(
                    "Y_squared_norm", torch.linalg.matrix_norm(train_y) ** 2
                )  # case of the PLMC_fast (term for MLL computation)
        elif diagonal_B:
            self.register_parameter(
                "log_Sigma_orth",
                torch.nn.Parameter(log_init_noise * discarded_noise_tens),
            )
            self.register_constraint(
                "log_Sigma_orth", gp.constraints.GreaterThan(log_noise_thresh)
            )
            ...
            self.register_parameter(
                "Sigma_orth_inv_chol",
                torch.nn.Parameter(
                    torch.diag_embed(1 / noise_init * discarded_noise_tens)
                ),
            )
            torch.nn.utils.parametrize.register_parametrization(
                self,
        ...
        if not BDN and n_tasks > n_latents:
            self.register_parameter(
                "M",
                torch.nn.Parameter(
                    torch.zeros([*batch_shape, n_latents, n_tasks - n_latents])
                ),
            )
    ...
    def projection_matrix(self) -> Tensor:
        ...
        H_pinv = torch.linalg.solve_triangular(
            R.mT, Q, upper=False, left=False
        )  # shape (n_batch x) n_tasks x n_latents
        if self._has_M_term:
    ...
    def project_data(
        self, data: Tensor, last_data_dim_is_datapoint: bool = False
    ) -> Tensor:
        """
        ...
        if self._has_M_term:
            ...
            res = (
                Hpinv_times_Y
                + self.projected_noise().unsqueeze(-1) * self.M @ orthog_proj
            )
        else:
            res = Hpinv_times_Y
    ...
    def full_likelihood(
        ...
        gp.likelihoods.MultitaskGaussianLikelihood,
        NonfactoredMultitaskGaussianLikelihood,
    ]:
        """
        ...
        if self.n_latents < self.n_tasks:
            if self.scalar_B:
                Sigma_orth = (
                    Sigma_orth_root**2
                )  # Only taske the scalar value, not the full vector (but same number of dimensions)
                if diag:
                ...
                if self.diagonal_B:
                    ...
                    if self._has_M_term:
                        Sigma_orth = (
                            Sigma_orth_root**2
                        )  # shape (*batch_shape) x (n_tasks - n_lat)
                else:
                    B_term_root = Q_orth @ Sigma_orth_root
                    if self._has_M_term:
                        Sigma_orth = Sigma_orth_root @ Sigma_orth_root.mT
                B_term = (
                    B_term_root @ B_term_root.mT
                    if not diag
                    else (B_term_root**2).sum(dim=-1)
                )
        else:
            B_term = 0.0
        ...
        if self._has_M_term:
            if self.diagonal_B:
                M_term = (
                    -QR @ ((sigma_p.unsqueeze(-1) * self.M) * Sigma_orth) @ Q_orth.mT
                )
            ...
            D_term = (
                D_term_root @ D_term_root.mT
                if not diag
                else (D_term_root**2).sum(dim=-1)
            )

        if diag:
            res = gp.likelihoods.MultitaskGaussianLikelihood(
                num_tasks=self.n_tasks,
                batch_shape=self.shape_batch,
                rank=0,
                has_global_noise=False,
            )
            if sigma_p.is_cuda:
                res.cuda()
            diag_M_term = (
                torch.diagonal(M_term, dim1=-2, dim2=-1) if self._has_M_term else 0.0
            )
            diag_noises = B_term + D_term + 2 * diag_M_term
            res.task_noises = torch.clamp(diag_noises, min=2 * self.noise_thresh)
        else:
            Mt_term = M_term.mT if self._has_M_term else 0.0
            Sigma = D_term + M_term + Mt_term + B_term
            res = NonfactoredMultitaskGaussianLikelihood(
                num_tasks=self.n_tasks,
                batch_shape=self.shape_batch,
                task_noise_covar=Sigma,
            )
            if sigma_p.is_cuda:
                res.cuda()
    ...
    def Sigma_orth_root(self) -> Tensor:
        ...
        if self.n_latents < self.n_tasks:
            if self.diagonal_B:
                ...
                identities = torch.broadcast_to(
                    torch.eye(
                        discarded_noise_size, device=self.Sigma_orth_inv_chol.device
                    ),
                    (*self.shape_batch, discarded_noise_size, discarded_noise_size),
                )
                res = torch.linalg.solve_triangular(
                    self.Sigma_orth_inv_chol, identities, upper=False
                ).mT  # TOSEE : is this mT legit ?
        else:
            res = torch.Tensor()
        return res
    ...
    ):  # ! forward only returns values of the latent processes !
        """
    ...
    def compute_latent_distrib(
        ...
        Returns:
            A batched gp multivariate normal distribution representing latent processes values, which mean has shape n_latents x n_points.
        """
        proj_targets = self.project_data(
            self.train_y, last_data_dim_is_datapoint=self.last_target_dim_is_datapoint
        )
        super().set_train_data(
            inputs=self.train_inputs, targets=proj_targets, strict=False
        )
    ...
    def set_train_data(
        ...
        targets: Tensor,
        strict: bool = True,
        last_target_dim_is_datapoint: Union[bool, None] = None,
    ):
        """
        ...
        projected_data = self.project_data(
            targets, last_data_dim_is_datapoint=last_target_dim_is_datapoint
        )
        super().set_train_data(inputs=inputs, targets=projected_data, strict=strict)
        self.train_y = (
            targets
            if self.last_target_dim_is_datapoint == last_target_dim_is_datapoint
            else targets.mT
        )
    ...
    def save(self, extra_terms=False) -> dict:
        ...
        dico = {}
        dico["kernel_type"] = (
            self.covar_module.base_kernel.__class__.__name__
            if self.outputscales is None
            else self.covar_module.__class__.__name__
        )
        with torch.no_grad():
            if extra_terms:
                dico["noise_thresh"] = (
                    self.likelihood.raw_noise_constraint.lower_bound.item()
                )
    ...
    def __call__(
        ...
        super().set_train_data(
            inputs=self.train_inputs, targets=projected_data, strict=False
        )
...
class ProjectedLMCmll(gp.mlls.ExactMarginalLogLikelihood):
    ...
    def __init__(
        self,
        latent_likelihood: Likelihood,
        model: ProjectedGPModel,
        last_target_dim_is_datapoint: Union[bool, None] = None,
    ):
        """
        ...
        if not isinstance(
            latent_likelihood,
            gp.likelihoods.gaussian_likelihood._GaussianLikelihoodBase,
        ):
    ...
    def forward(
        self,
        latent_function_dist: gp.distributions.Distribution,
        target: Tensor,
        inputs=None,
        *params,
    ) -> Tensor:
        ...
        """
        if not isinstance(
            latent_function_dist,
            gp.distributions.multivariate_normal.MultivariateNormal,
        ):
            raise RuntimeError(
        ...
        # project the targets
        proj_target = self.model.project_data(
            target, last_data_dim_is_datapoint=False
        )  # shape (n_batch x) n_latents x n_points

        # Get the log prob of the marginal distribution of latent processes
        latent_output = self.likelihood(
            latent_function_dist, *params
        )  # shape (n_batch x) n_latents x n_points
        latent_res = latent_output.log_prob(proj_target)
        latent_res = (
            self._add_other_terms(latent_res, params).sum() / num_data
        ...
        if not self.model._has_M_term and self.model.scalar_B:  # case of the PLMC-fast
            if self.model.log_Sigma_orth.numel() > 0:
                ...
                self.proj_term_list[1] = (
                    scalar_Sigma_orth_inv
                    * (
                        self.model.Y_squared_norm
                        - torch.linalg.matrix_norm(target @ Q) ** 2
                    )
                ).sum() / num_data
                # the parenthesis is the squared norm of the projection of target onto the space orthogonal to span(Q)
            else:
                self.proj_term_list[1] = 0.0
                log_Sigma_orth_root_diag = torch.tensor([0.0])
        else:
            if self.model.diagonal_B:
                log_Sigma_orth_root_diag = self.model.log_Sigma_orth / 2
                rot_proj_scaled_target = (
                    target @ Q_orth * torch.exp(-log_Sigma_orth_root_diag)
                )
            else:
                Sigma_orth_inv_root_diag = self.model.Sigma_orth_inv_chol[
                    ..., range(p - q), range(p - q)
                ]
                log_Sigma_orth_root_diag = -torch.log(Sigma_orth_inv_root_diag)
                rot_proj_scaled_target = (
                    target @ Q_orth @ self.model.Sigma_orth_inv_chol
                )
            self.proj_term_list[1] = (
                torch.linalg.matrix_norm(rot_proj_scaled_target) ** 2 / num_data
            )

        # All terms are implicitly or explicitly divided by the number of datapoints
        self.proj_term_list[0] = 2 * torch.sum(
            log_Sigma_orth_root_diag
        )  # factor 2 because of the use of a root

        if self.model.lmc_coefficients.bulk:
            self.proj_term_list[2] = torch.log(
                R[..., range(q), range(q)] ** 2
            ).sum()  # keep the square in the log because the quantity can be negative
        else:
            self.proj_term_list[2] = (
                2
                * self.model.lmc_coefficients.parametrizations.R.original[
                    ..., range(q), range(q)
                ].sum()
            )
