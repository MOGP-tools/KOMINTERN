...
class BayesPolyRegressor:
    def __init__(self, n_tasks, degree, tol, max_iter, model=BayesianRidge, **kwargs):
        ...
        self.models = [
            make_pipeline(
                ...
                model(tol=tol, compute_score=False, max_iter=max_iter, **kwargs),
            )
    ...
    def train(self, train_x, train_y, compute_score=True, coeff_info=False):
        ...
        for i in range(train_y.shape[1]):
            ...
            if i == 0:
                ...
                )  # The model must be fitted for this to be defined.
                coeff_list = np.zeros((n_funcs, len(monomials)))

            reg = model.steps[1][
                1
            ]  # reg is the regression part of the model (not the polynomial features)
            n_coeffs[i] = len(reg.coef_[reg.coef_ != 0.0])
            if coeff_info:
            ...
            if compute_score:
                ...
                model.steps[1][
                    1
                ].compute_score = False  # Resetting the compute_score flag to default
    ...
    def predict(self, test_x, return_std=False):
        ...
        for i in range(n_funcs):
            y_pred[:, i], y_pred_std[:, i] = self.models[i].predict(
                test_x, return_std=return_std
            )
