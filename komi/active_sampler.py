...
from komi.utilities import CorrectReduceLROnPlateau
...
class ActiveSampler:
    ...
    def modify_train_set(self, new_X=None, new_Y=None, update_inducing_points=False):
        if new_X is not None and new_Y is not None:
            ...
            if self.strategy == "downsampling":
                new_entries = torch.ones(
                    len(new_X), dtype=torch.bool, device=self.mask.device
                )
        ...
        if update_inducing_points:
            ...
            variational_dist = var_dist_class(
                n_points, batch_shape=torch.Size([self.model.n_latents])
            )
            self.model.variational_strategy.base_variational_strategy._variational_distribution = variational_dist

    def find_next_points(
        self, n_samples, output_train=None, only_idx=False, verbose=True, **kwargs
    ):
        # 1. Compute Scores for ALL points
        if self.strategy == "downsampling":
            var_loo, delta_loo = self.model.compute_loo(
                output_train, latent=(self.sampling_domain == "latent")
            )
            ...
            if self.strategy == "variance":
                ...
                scores = self.compute_loo_scores(
        ...
        # print(self.model.task_variance(self.X_candidates))
        # print('var_values', var_values)
        # print('scores', scores)
        # print('eval_points', eval_points)
        # if self.current_X is not None and len(self.current_X) > 0:
        #     dist_matrix = cdist(eval_points.cpu().numpy(), self.current_X.cpu().numpy())
        #     print('distance_matrix', dist_matrix)
        ...
        masked_scores = (
            scores.clone()
            .detach()
            .masked_fill_(~self.mask.to(scores.device), fill_value)
        )
        # print(masked_scores)

        # 3. Retrieve Top K
        # largest=False for downsampling (remove least useful), True for active learning (pick most uncertain)
        top_values, top_ind = torch.topk(
            masked_scores, n_samples, largest=(not self.strategy == "downsampling")
        )
        # print(top_values)
        # print(top_ind)
        ...
        if n_samples == 1 and not only_idx:
            new_points = (
                new_points.unsqueeze(0)
                if torch.is_tensor(new_points)
                else np.expand_dims(new_points, 0)
            )
    ...
    def train_model(
        ...
        if self.scheduler is None or reset_optimizer:
            self.scheduler = CorrectReduceLROnPlateau(
                self.optimizer,
                factor=0.5,
        ...
        for i in range(n_iters):
            ...
            # Improvement checks
            loss_better_than_best = (
                new_loss >= 0.0 and new_loss < best_loss * (1 - loss_thresh)
            ) or (new_loss < 0.0 and new_loss < best_loss * (1 + loss_thresh))
            loss_almost_as_good_as_best = (
                new_loss >= 0.0 and new_loss < best_loss * (1 + loss_thresh)
            ) or (new_loss < 0.0 and new_loss < best_loss * (1 - loss_thresh))
            ...
            if (
                use_stop
                and (no_improve_count > patience_crit)
                and loss_almost_as_good_as_best
            ):
                effective_iters = i
                # print("Sched thresh :", self.scheduler._is_better(new_loss, best_loss, return_thresh=True))
                break
        self.model.eval()
        return new_loss, effective_iters

    def compute_loo_scores(
        self, var_values, lscales_mat, e_loo2, s_loo2, aggregated=True
    ):
        space_size = lscales_mat.shape[0]
        weights = torch.zeros((len(self.X_candidates), space_size))
        for i in range(space_size):
            dist_mat = cdist(
                self.X_candidates.cpu().numpy() / lscales_mat[i, :].cpu().numpy(),
                self.current_X.values / lscales_mat[i, :].cpu().numpy(),
            )
