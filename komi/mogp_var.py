...
class CustomLMCVariationalStrategy(gp.variational.LMCVariationalStrategy):
    ...
    def __call__(
        self, x: Tensor, task_indices=None, prior=False, **kwargs
        ...
        return multitask_dist.__class__(
            multitask_dist.mean + tasks_means.mT, multitask_dist.lazy_covariance_matrix
        )
...
class VariationalMultitaskGPModel(gp.models.ApproximateGP):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if float(train_ind_ratio) == 1.0:
            warnings.warn("Caution : inducing points not learned !")
            inducing_points = train_x
            learn_inducing_locations = False
            var_strat = (
                gp.variational.UnwhitenedVariationalStrategy
            )  # better compatibility in this case
            distrib = (
                gp.variational.CholeskyVariationalDistribution
            )  # better compatibility in this case
        else:
            learn_inducing_locations = True
            n_ind_points = int(np.floor(n_points / train_ind_ratio))
            inducing_points = initialize_inducing_points(
                X=train_x, M=n_ind_points, with_qmc=init_induc_with_qmc, seed=seed
            )
            # same inducing points for all latents here

        variational_distribution = distrib(
            inducing_points.size(-2), batch_shape=latent_batch_shape
        )
        strategy = var_strat(
            self,
            inducing_points,
        ...
        self.mean_module = gp.means.ZeroMean(
            batch_shape=latent_batch_shape
        )  # in gp, latent processes can have non-zero means, which we wish to avoid
        ...
        if init_lmc_coeffs:
            ...
            self.variational_strategy.lmc_coefficients = torch.nn.Parameter(
                lmc_coefficients
            )  # shape (n_batch x) n_latents x n_tasks
    ...
    ) -> Union[List[Tensor], Tensor]:  # returned shape : n_kernels x n_dims
        """
    ...
    def compute_latent_distrib(self, x, prior=False, **kwargs):
        ...
        Returns:
            A batched gp multivariate normal distribution representing latent processes values, which mean has shape n_latents x n_points.
        """
        return self.variational_strategy.base_variational_strategy(
            x, prior=prior, **kwargs
        )
    ...
    def kernel_cond(self) -> Tensor:
        ...
        with torch.no_grad():
            ...
            K_plus = self.variational_strategy.base_variational_strategy.pseudo_points[
                0
            ]
    ...
    def save(self, extra_terms=False) -> dict:
        ...
        with torch.no_grad():
            ...
            inducing_points = (
                self.variational_strategy.base_variational_strategy.inducing_points
            )
            ...
            if not isinstance(
                self.variational_strategy.base_variational_strategy,
                gp.variational.UnwhitenedVariationalStrategy,
            ):
                warnings.warn(
                    "Model storage has only been tested for the UnwhitenedVariationalStrategy of gpytorch. \
                            The current strategy may not be supported."
                )
            dico["mean_cache"] = (
                self.variational_strategy.base_variational_strategy._mean_cache.squeeze().tolist()
            )
            ...
            if extra_terms:
                ...
                if isinstance(
                    self.variational_strategy.base_variational_strategy._variational_distribution,
                    gp.variational.CholeskyVariationalDistribution,
                ):
                    dico["distrib_covar"] = (
                        self.variational_strategy.base_variational_strategy._variational_distribution.chol_variational_covar.tolist()
                    )
                else:
                    warnings.warn(
                        "The variational covariance was not stored, either deliberately (delta variational distribution) or because a nonstandard distribution was used."
                    )
        return dico

    def default_mll(self):
        if self.last_target_dim_is_datapoint:
            return TransposedVariationalELBO(
                self.likelihood, self, num_data=self.n_points
            )
        else:
            return gp.mlls.VariationalELBO(
                self.likelihood, self, num_data=self.n_points
            )
