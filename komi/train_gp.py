...
def train_model(model_init, X, Y, argus, optimizer=None, compute_loo=False, return_optim=False, met_dict={}, extra_context_managers={}, verbose=True):
    try:
        if len(Y.shape) == 1:
            ...
            with torch.no_grad():
                ...
                if not met_dict: # if no metrics are specified, we return the raw data
                    ...
                    raw_metrics = {'y_test':Y, 'deltas':loo_deltas, 'errs':torch.abs(loo_deltas), 'errs2':loo_deltas**2,
                                'sigmas':torch.sqrt(loo_vars), 'vars':loo_vars}
                    for met in met_dict:
...
def eval_model(model, X_test, Y_test, argus, met_dict, extra_context_managers={}, verbose=True):
        ...
        if argus['gpu']:
            ...
            for i in range(0, len(X_test), batch_size):
                ...
                if not (context_managers.get('skip_pred_var', True) and isinstance(model, MultitaskGPModel)): 
                    ...
                    sigma_pred = vars_pred.sqrt().squeeze()
                    stds.append(sigma_pred)
    ...
    raw_metrics = {'y_test':Y_test, 'deltas':deltas, 'errs':torch.abs(deltas), 'errs2':deltas**2, 'sigmas':sigma_pred, 'vars':vars_pred}
    for met in met_dict:
        metrics[met] = met_dict[met](raw_metrics).cpu().numpy()
    return pred_y, sigma_pred, metrics


def eval_loo(model, Y_train, met_dict={}, extra_context_managers={}):
    start = time.time()
    context_managers = train_context_managers.copy()
    context_managers.update(extra_context_managers)
...
def train_parallel(model_list, X, Y, argus, max_workers, compute_loo=False, met_dict={}, 
...
def eval_parallel(model_list, X_test, Y_test, argus, max_workers, met_dict={}, extra_context_managers={}):
    n_tasks = Y_test.shape[1]
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
    ...
    raw_metrics = {'y_test':Y_test, 'deltas':deltas, 'errs':torch.abs(deltas), 
                   'errs2':deltas**2, 'sigmas':sigma_preds, 'vars':sigma_preds**2}
    for met in met_dict:
        metrics[met] = met_dict[met](raw_metrics).cpu().numpy()
