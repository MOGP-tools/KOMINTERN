...
from .utilities import TruncatedRBFCovariance, truncated_exponential
from gpytorch.settings import trace_mode
from gpytorch.kernels.kernel import Kernel


class TruncatedRBFKernel(Kernel):
    r"""
    ...
    def forward(self, x1, x2, diag=False, **params):
        if (
            ...
            return truncated_exponential(
                self.covar_dist(x1_, x2_, square_dist=True, diag=diag, **params).div(-2),
                k=self.order)
        return TruncatedRBFCovariance.apply(
            x1,
            x2,
            self.lengthscale,
            self.order,
