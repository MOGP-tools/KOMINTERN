...
from komi.lowrank_multitask_kernel import LowrankMultitaskKernel
from komi.utilities import compute_truncated_svd
...
class MultitaskGPModel(ExactGPModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(MultitaskGPModel, self).__init__(
            train_x, train_y, likelihood, ignore_n_tasks=True, batch_lik=False, **kwargs
        )
        ...
        if model_type == "ICM":
            if lowrank:
                self.covar_module = LowrankMultitaskKernel(
                    self.covar_module, num_tasks=n_tasks, rank=n_latents
                )
            else:
                self.covar_module = gp.kernels.MultitaskKernel(
                    self.covar_module, num_tasks=n_tasks, rank=n_latents
                )
            self.covar_module.task_covar_module.var = torch.tensor([1.0])
        elif model_type == "LMC":
            self.covar_module = gp.kernels.LCMKernel(
                base_kernels=[
                    copy.deepcopy(self.covar_module) for i in range(n_latents)
                ],
                num_tasks=n_tasks,
                rank=1,
            )
        ...
        if init_lmc_coeffs:
            ...
            if model_type == "ICM":
                ...
                self.covar_module.task_covar_module.covar_factor = torch.nn.Parameter(
                    lmc_coeffs
                )
            elif model_type == "LMC":
                for i in range(n_latents):
                    # this parameter has already been initialized with random values at the instantiation of the variational strategy, so registering it anew is facultative
                    self.covar_module.covar_module_list[
                        i
                    ].task_covar_module.covar_factor = torch.nn.Parameter(
                        lmc_coeffs[:, i].unsqueeze(-1)
                    )
            else:
    ...
    def lmc_coefficients(self) -> Tensor:
        ...
        if self.model_type == "LMC":
            ...
            for i in range(self.n_latents):
                res[i] = self.covar_module.covar_module_list[
                    i
                ].task_covar_module.covar_factor.data.squeeze()
        else:
    ...
    def lscales(self, unpacked: bool = True) -> Union[List[Tensor], Tensor]:
        ...
        if hasattr(data_covar, "kernels"):
            ...
            if self.model_type == "ICM":
                scales = [
                    reduce(getattr, attr_name.split("."), ker)
                    .squeeze()
                    .repeat(self.n_latents, 1)
                    for ker in data_covar.kernels
                ]
            else:
                ref_scales = [
                ...
                ker_dims = [
                    len(scales) if scales.ndim > 0 else 1 for scales in ref_scales
                ]
                scales = [
                    torch.zeros((self.n_latents, ker_dims[i])) for i in range(n_kernels)
                ]
        else:
            n_kernels = 1
            ...
            if self.model_type == "ICM":
                scales = (
                    reduce(getattr, attr_name.split("."), data_covar)
                    .squeeze()
                    .repeat(self.n_latents, 1)
                )
            else:
                ...
                if n_kernels > 1:
                    for j in range(n_kernels):
                        scales[j][i, :] = reduce(
                            ...
                            self.covar_module.covar_module_list[
                                i
                            ].data_covar_module.kernels[j],
                        ).squeeze()
                else:
                    scales[i, :] = reduce(
                        getattr,
                        attr_name.split("."),
                        self.covar_module.covar_module_list[i].data_covar_module,
                    ).squeeze()
    ...
    def outputscale(self, unpacked: bool = False) -> Tensor:
        ...
        if n_kernels > 1:
            for i_ker in range(n_kernels):
                if self.model_type == "LMC":
                    for i_lat in range(self.n_latents):
                        res[i_lat, i_ker] = (
                            self.covar_module.covar_module_list[i_lat]
                            .data_covar_module.kernels[i_ker]
            ...
            if self.model_type == "LMC":
                for i_lat in range(self.n_latents):
                    res[i_lat, 0] = self.covar_module.covar_module_list[
                        i_lat
                    ].data_covar_module.outputscale.data.squeeze()
            else:
    ...
    def compute_var(self, x):
        ...
        linop = self.covar_module.forward(x, x) + self.likelihood._shaped_noise_covar(
            (len(x), len(x)), add_noise=True
        )
        ...
        noise_op = self.likelihood._shaped_noise_covar(
            (len(x_train), len(x_train)), add_noise=True
        )
        i_task = (
            1
            if isinstance(
                ker_op.linear_ops[1], (PsdSumLinearOperator, RootLinearOperator)
            )
            else 0
        )  # TODO : clean this
        ...
        for i in range(0, n, batch_size):
            ...
            k_hat = (
                self.covar_module.data_covar_module(x_batch, x_train)
                .matmul(k_evecs)
                .to_dense()
                .squeeze()
            )
            ...
            second_term_results.append(
                second_term.reshape((len(x_batch), self.n_tasks))
            )
    ...
    def compute_loo(self, output=None):
        ...
        with gp.settings.cholesky_max_tries(6), torch.no_grad():
            if output is None:
            ...
            targets = torch.reshape(
                train_y - m, (np.prod(train_y.shape).astype(int), 1)
            )
    ...
    def kernel_cond(self) -> Tensor:
        ...
        with torch.no_grad():
            ...
            K_plus_noise = K + torch.eye(
                len(self.train_inputs[0]), device=self.train_inputs[0].device
            )
    ...
    def save(self, extra_terms=False):
        ...
        with torch.no_grad():
            ...
            if extra_terms:
                ...
                dico["noise_thresh"] = (
                    likelihood.raw_noise_constraint.lower_bound.item()
                )
                noises = likelihood.noise if hasattr(likelihood, "noise") else 0.0
