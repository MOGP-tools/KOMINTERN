...
init_tag = 'large-dataset' if strategy in ['Ldownsampling', 'Tdownsampling'] else 'small-dataset'
...
## Candidate data (dummy mode only)
dummy_mode = True
...
first_run = True
optimizer = None # For the first iteration, the optimizer is not defined. Later, it is stored at each iteration
for k in points_iter:
    ...
    if not dummy_mode or strategy in ['Tdownsampling', 'Ldownsampling']:
        ...
        for i in range(k, k + batch_size):
            ...
            xs_data = pd.read_csv(xs_paths[-1] + study_tag_base + '/' + assembly_name + '_' + hom_name + '_' + study_tag + '_xs.csv', index_col=0) # modify as needed
            cc_data = pd.read_csv(xs_paths[-1] + study_tag_base + '/' + assembly_name + '_' + hom_name + '_' + study_tag + '_cc.csv', index_col=0) # modify as needed
            xs_data_res.update({i-1 : xs_data.to_dict(orient='index')['0_0']}) #  The 0_0 string is due to the export format of the HXS csv's
            cc_data_res.update({i-1 : cc_data.to_dict(orient='index')['0_0']})
        ...
        if strategy in ['Tdownsampling', 'Ldownsampling']:
            ...
            for i in range(k, k + batch_size):
                if batch_size==1:
                    xs_data_res.update({i-1 : part_xs.to_dict()})
                    cc_data_res.update({i-1 : part_cc.to_dict()})
                else:
                    xs_data_res.update({i-1 : part_xs.to_dict(orient='index')['0_0']})
                    cc_data_res.update({i-1 : part_cc.to_dict(orient='index')['0_0']})
